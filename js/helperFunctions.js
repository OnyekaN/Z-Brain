...
function preloadDataset(dataset, length) {
	sliceNo = 45
	var images = new Array()
	for ( var i = 0 ; i < length ; i++ ) {
	  images[i] = new Image() 
	  images[i].src = "Images/"+dataset+"-"+i+".jpg" 
	}	
	return images
}

function cachePreloadDataset(dataset, slice, length) {
	
	var i = 0 ;
	var images = new Array()
	while ( slice + i < length || slice - i > -1 ) {
	
	  if ( slice + i < length ) {
	     var j = slice + i ;
	     images[j] = new Image()
	     images[j].src = "Images/"+dataset+"-"+j+".jpg" 
	  }
	  if ( slice - i > - 1  ) {	
	     var k = slice - i ;
	     images[k] = new Image()
	     images[k].src = "Images/"+dataset+"-"+k+".jpg"
	  }
	  i++	
	}
	return images
...
function maskFormatNum(num, size) {
	var s = num+"";
	while ( s.length < size )
		s = "0" + s;
	return s
...
function preloadMask(mask, color, length) {
	var images = new Array()
	for ( var i = 1 ; i <= length ; i++ ) {
	  images[i-offset] = new Image()
	  num = maskFormatNum(i, 3)
	  imgsrc = String("Masks/"+color+"/"+mask+"_slice"+num+".png") 
	  images[i-offset].src = imgsrc	 
	}
	return images
}
...
function buttonLabel(color, mask) {

	var oldName = mask //activeMasks[color].mask,
	    formattedName = {}, 
